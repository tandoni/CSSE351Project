...
public:
	...
	void init(char const * path)
	{
		...
		glm::translate(glm::mat4(1), -this->getCentroid());
	...
	void init(char const * path, char const * VertPath, char const * FragPath)
	{
		init(path);
		setShader(VertPath,FragPath);
	}
	...
	void draw()
	{
		...
		for (size_t matGroup = 0; matGroup<switchMaterialAt.size() - 1; matGroup++)
		{
			...
			glUniform4fv(glGetUniformLocation(shaderProg, "Ka"), 1, &materials[matId].Ka[0]);
			glUniform4fv(glGetUniformLocation(shaderProg, "Kd"), 1, &materials[matId].Kd[0]);
			glUniform4fv(glGetUniformLocation(shaderProg, "Ks"), 1, &materials[matId].Ks[0]);
			glUniform4fv(glGetUniformLocation(shaderProg, "specAlpha"), 1, &materials[matId].specAlpha);
	...
	void setShader(char const * Vertpath, char const * Fragpath)
	{
		char const * vertPath = shaderProg+".vert";
		char const * fragPath = shaderProg+".frag";
